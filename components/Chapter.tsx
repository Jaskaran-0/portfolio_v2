...
export default function Chapter({ num, title, quote, id, children }: ChapterProps) {
  ...
  const yellowRuleRef = useRef<HTMLDivElement>(null)
      ...
      entries => {
        if (entries[0].isIntersecting && !triggered.current) {
          ...
          chapterCardEntrance({
            ...
            yellowRule: yellowRuleRef.current,
            quote:      quoteRef.current || null,
            bottomRule: bottomRuleRef.current,
          })
        }
      },
      { threshold: 0.12 }
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 2, maxWidth: 860 }}>
        ...
        <div
          ref={topRuleRef}
          style={{ height: 1, background: 'var(--red)', marginBottom: 20, transformOrigin: 'left', opacity: 0 }}
        />

        <div style={{ display: 'flex', gap: 'clamp(16px, 3vw, 36px)', alignItems: 'flex-start' }}>
          {/* Vertical line */}
          <div
            ref={vertLineRef}
            style={{ width: 2, background: 'var(--red)', alignSelf: 'stretch', minHeight: 80, transformOrigin: 'top', opacity: 0, flexShrink: 0 }}
          />
          ...
          <div style={{ flex: 1 }}>
            ...
            <div
              ref={yellowRuleRef}
              style={{ height: 2, background: 'var(--yellow)', width: 48, marginBottom: 16, transformOrigin: 'left', opacity: 0 }}
            />
        ...
        <div
          ref={bottomRuleRef}
          style={{ height: 1, background: 'var(--red)', marginTop: 20, transformOrigin: 'left', opacity: 0 }}
        />
      </div>
    </section>
  )
